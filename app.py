...
def load_model_artifacts():
    ...
    model, le_dict, le_target, scaler, feature_names = None, None, None, None, None
            ...
            if os.path.exists(scaler_path):
                try:
                    ...
                    st.success("✅ Scaler loaded successfully!")
                     ...
                     st.warning(f"⚠️ Could not load scaler from {scaler_path}: {e}")
                     scaler = None # Ensure scaler is None if loading fails
                     traceback.print_exc() # Print traceback for scaler loading error


            st.success("✅ Pre-trained model and preprocessors loaded successfully!")
            return model, le_dict, le_target, scaler, feature_names
        except (FileNotFoundError, pickle.UnpicklingError, Exception) as e: # Catch specific pickle errors and general Exception
            st.error(f"❌ Error loading one or more model artifacts: {e}")
