...
def init_session():
    ...
    if 'form_data' not in st.session_state:
        st.session_state['form_data'] = {
            'age': 5, 'breed': None, 'sex': None, 'calvings': 1, 'abortion': None,
            'infertility': None, 'vaccine': None, 'sample': None, 'test': None,
            'retained': None, 'disposal': None
        }
...
def handle_chat_submit():
    if st.session_state.chat_input:
        ...
        try:
            ...
            Provide concise advice in {'Hindi' if lang == 'Hindi' else 'English'}."""
...
# --- CORE LOGIC (OTP, Sheets, Model Load) ---
...
def verify_otp(entered_otp):
    if st.session_state.get('otp_code') == entered_otp:
        return True, "Success"
...
def register_user(email, password, name, phone, location):
    try:
        ...
        if email in users: return False, "User exists"
...
if not st.session_state['logged_in']:
    # --- LOGIN / SIGNUP UI (Skipped for brevity, same as yours) ---
    ...
    with tab1:
        with st.form("login_form"):
            ...
            if st.form_submit_button("Access Dashboard"):
                # simplified check
                st.session_state.update(logged_in=True, username=lemail)
                create_user_session(lemail)
                st.rerun()
else:
    # --- SIDEBAR ---
    ...
    with st.sidebar:
        ...
        if ai_enabled:
            if st.button(f"🤖 {t['ai_assistant']}"):
                st.session_state['show_chatbot'] = not st.session_state['show_chatbot']
                # Removed st.rerun() here as button click handles it
    ...
    with col_left:
        ...
        with st.form("prediction_form"):
            # ... Input Logic (Same as your code) ...
            c_a, c_b = st.columns(2)
            with c_a:
                age = st.number_input(t["age"], 0, 20, 5)
                # Note: Breed/Sex selectboxes using le_dict logic
            with c_b:
                vaccine = st.selectbox(t["vaccination"], ["Yes", "No"])
            
            if st.form_submit_button(f"🔬 {t['run_prediction']}"):
                # Mock result for logic flow
                st.session_state['last_prediction'] = {'result': 'Negative', 'confidence': 0.95, 'classes': ['Pos', 'Neg'], 'probabilities': [0.05, 0.95]}

        if st.session_state.get('last_prediction'):
            st.success("Prediction Ready") # Chart logic remains same
            
    with col_right:
        # --- FIXED CHATBOT SECTION ---
        if ai_enabled:
            ...
            if st.session_state['show_chatbot']:
                # Use a container for messages to prevent page jumping
