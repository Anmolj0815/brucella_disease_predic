...
def generate_pdf_report(prediction, ai_insight, chat_history, username):
    ...
    try:
        ...
        pdf.cell(0, 15, 'BrucellosisAI - Prediction Report', 0, 1, 'C')
        pdf.set_font('Arial', '', 10)
        pdf.cell(0, 8, f'Generated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}', 0, 1, 'C')
        pdf.cell(0, 8, f'User: {username}', 0, 1, 'C')
        pdf.ln(10)
        
        # Prediction Result
        pdf.set_font('Arial', 'B', 14)
        pdf.set_fill_color(240, 240, 240)
        pdf.cell(0, 10, 'PREDICTION RESULT', 0, 1, 'L', True)
        ...
        pdf.cell(0, 10, f'Status: {result}', 0, 1)
        ...
        if probs and classes:
            ...
            for cls, prob in zip(classes, probs):
                pdf.cell(0, 6, f'  - {cls}: {prob:.1%}', 0, 1)
        pdf.ln(5)
        
        # Input Data
        pdf.set_font('Arial', 'B', 14)
        pdf.cell(0, 10, 'INPUT DATA', 0, 1, 'L', True)
        pdf.set_font('Arial', '', 10)
        ...
        if ai_insight:
            ...
            pdf.multi_cell(0, 6, ai_insight.encode('latin-1', 'replace').decode('latin-1'))
            pdf.ln(5)
        
        # Chat History
        if chat_history and len(chat_history) > 0:
            pdf.set_font('Arial', 'B', 14)
            pdf.cell(0, 10, 'AI CONSULTATION HISTORY', 0, 1, 'L', True)
            pdf.set_font('Arial', '', 10)
        ...
        pdf.cell(0, 5, 'This report is generated by BrucellosisAI Prediction System.', 0, 1, 'C')
        pdf.cell(0, 5, 'For veterinary use only. Please consult a licensed veterinarian for diagnosis.', 0, 1, 'C')
        
        # Return PDF as bytes (fpdf2 returns bytes directly)
...
def generate_text_report(prediction, ai_insight, chat_history, username):
    """Generate a text report as fallback if PDF not available"""
    report = []
    report.append("=" * 60)
    report.append("BRUCELLOSISAI - PREDICTION REPORT")
    report.append("=" * 60)
    ...
    # Prediction Result
    report.append("-" * 40)
    report.append("PREDICTION RESULT")
    report.append("-" * 40)
    report.append(f"Status: {prediction.get('result', 'N/A')}")
    report.append(f"Confidence: {prediction.get('confidence', 0):.1%}")
    ...
    report.append("-" * 40)
    for key, value in prediction.get('input_data', {}).items():
    ...
    if ai_insight:
        ...
        report.append("-" * 40)
        report.append(ai_insight)
        report.append("")
    
    # Chat History
    if chat_history and len(chat_history) > 0:
        report.append("-" * 40)
        report.append("AI CONSULTATION HISTORY")
        report.append("-" * 40)
    ...
    report.append("")
    report.append("=" * 60)
    report.append("This report is for veterinary use only.")
    report.append("Please consult a licensed veterinarian for diagnosis.")
...
# ============================================
    ...
    with col_right:
        ...
        with st.expander(f"📄 {t['export_report']}", expanded=False):
            if pred:
                st.write("**Report includes:**")
                st.write("✅ Prediction Result & Confidence")
                st.write("✅ All Input Data")
                if st.session_state.get('ai_insight'):
                ...
                ai_insight = st.session_state.get('ai_insight', None)
                chat_history = st.session_state.get('chat_history', [])
                
                # PDF Download
                if PDF_AVAILABLE:
                    pdf_data = generate_pdf_report(pred, ai_insight, chat_history, username)
                    if pdf_data:
                        st.download_button(
                            "📥 Download PDF Report",
                            data=pdf_data,
                            file_name=f"brucellosis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
                            mime="application/pdf",
                            use_container_width=True
                        )
                    else:
                        st.warning("PDF generation failed. Try text report.")
                
                # Text Report as fallback
                text_report = generate_text_report(pred, ai_insight, chat_history, username)
                st.download_button(
                    "📥 Download Text Report",
                    data=text_report,
                    file_name=f"brucellosis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
                    mime="text/plain",
                    use_container_width=True
                )
            else:
                st.caption("Run a prediction first to generate report")
